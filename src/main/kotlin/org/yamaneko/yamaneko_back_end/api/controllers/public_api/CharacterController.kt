...
class CharacterController(
  ...
  fun getCharacters(): ResponseEntity<List<CharacterDTO>> {
    ...
    return if(characterList.isEmpty()) ResponseEntity.status(HttpStatus.NO_CONTENT).build()
    else ResponseEntity.status(HttpStatus.OK).body(characterList)
  }
  ...
  fun getCharacterById(
    @Parameter(
      description = "Enter id of character",
      example = "1"
    ) @PathVariable(required = false) characterId: Long
  ...
  fun saveCharacter(
    @RequestBody request: CharacterRequestDTO,
  ...
  fun patchCharacter(
    ...
    @RequestBody request: CharacterRequestPatch,
    httpRequest: HttpServletRequest
  ): ResponseEntity<Any> {
